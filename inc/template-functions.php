...
add_filter( 'body_class', 'mcstarter_body_classes' );
/**
 ...
 * @return array
 */
function mcstarter_body_classes( $classes ) {
	...
	if ( ! is_page() && ! is_404() && is_active_sidebar( 'sidebar-1' ) ) {
		$classes[] = 'sidebar';
		$classes[] = sanitize_html_class( get_theme_mod( 'mcstarter_sidebar_position', 'sidebar-right' ) ) ;
	} else {
		$classes[] = 'no-sidebar';
	}
	...
	if( has_post_thumbnail() || has_header_image() ){
		...
		$classes[] = sanitize_html_class( get_theme_mod( 'mcstarter_header_image_filter', 'raw-image' ) );
		$classes[] = sanitize_html_class( get_theme_mod( 'mcstarter_header_image_display', 'hero-image' ) );
	}
...
add_filter( 'post_class', 'mcstarter_post_classes', 10, 3 );
/**
 ...
 */
function mcstarter_post_classes( $classes, $class, $post_id ) {
	if( is_home() || is_archive() || is_search() ){
		$classes[] = sanitize_html_class( get_theme_mod( 'mcstarter_post_thumbnail_display', 'large-thumbnail' ) );
	}
	return $classes;
}


add_action( 'wp_head', 'mcstarter_pingback_header' );
/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function mcstarter_pingback_header() {
...
add_action( 'walker_nav_menu_start_el', 'mcstarter_social_icons', 10, 4 );
/**
 ...
 */
function mcstarter_social_icons( $item_output, $item, $depth, $args ) {
	...
	$supported_icons  = mcstarter_get_supported_social_icons();
	...
	if( $domain_or_scheme && array_key_exists( $domain_or_scheme, $supported_icons ) ){
		$svg = apply_filters( 'mcstarter_social_icon_svg', $supported_icons[$domain_or_scheme], $domain_or_scheme );
 ...
 **/ 
function mcstarter_get_supported_social_icons(){
	...
	return apply_filters( 'mcstarter_supported_social_icons' , $supported_icons );
 ...
 **/
function mcstarter_get_ui_icons(){
	...
	return apply_filters( 'mcstarter_ui_icons' , $ui_icons );

}


add_filter( 'the_title', 'mcstarter_default_post_title', 10, 2 );
/**
 ...
 **/
function mcstarter_default_post_title( $title, $id ){
	if ( empty( $title ) ){
		$title = '<em>' . __( 'Untitled', 'mcstarter' ). '</em>';
	}
	return $title;
}
 ...
 **/
function mcstarter_comment_callback( $comment, $args, $depth ) {
		...
		<div id="div-comment-<?php comment_ID(); ?>" class="comment-body wrapper comment-wrapper">
					...
					<strong class="comment-author-name h6"><?php comment_author_link( $comment ); ?></strong>
						<time class="comment-date" datetime="<?php comment_time( 'c' ); ?>">
							<?php
								...
								printf( esc_html__( 'On %1$s at %2$s', 'mcstarter' ), get_comment_date( '', $comment ), get_comment_time() );
							?>
						</time>
			...
			<div class="comment-content">
				<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'mcstarter' ); ?></p>
				<?php endif; ?>
				<?php comment_text(); ?>
			</div><!-- .comment-content -->
			...
			<footer class="comment-footer">
				<?php
					comment_reply_link( array_merge( $args, array(
						...
						'before'    => '<span class="reply-link">' . mcstarter_ui_icon( 'comment', false ),
						'after'     => '</span>'
					) ) );
					edit_comment_link( '<span>' . __( 'Edit', 'mcstarter' ) . '</span>', '<span class="edit-link">' . mcstarter_ui_icon( 'edit', false ), '</span>' ); 
				?>
 ...
 */
function mcstarter_jetpack_featured_image_archive_display() {
 ...
 */
function mcstarter_jetpack_featured_image_display() {
    if ( ! function_exists( 'jetpack_featured_images_remove_post_thumbnail' ) ) {
        return true;
    } else {
        $options         = get_theme_support( 'jetpack-content-options' );
